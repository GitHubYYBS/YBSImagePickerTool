...
@implementation YBSBubbleView{
    UIImageView *_iconImageView;
    UILabel *_titleLabel;
    BOOL _isShowing;// 是否正在显示中
    NSMutableDictionary<NSString * , YBSBubbleInfo *> *_infoDic;
    /// @brief 当前正在显示的泡泡信息对象
    YBSBubbleInfo *_currentInfo;
    /// @brief 当前自定义动画绘图的图层
...
static YBSBubbleView *defaultBubbleView;

+ (YBSBubbleView *)defaultBubbleView{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        defaultBubbleView = [[YBSBubbleView alloc] init];
    });
...
- (void)showWithInfo: (YBSBubbleInfo *)info{
    ...
    [UIView animateWithDuration: 0.4 delay:0 usingSpringWithDamping: 0.5 initialSpringVelocity:0.5 options: UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.frame = [info calBubbleViewFrame]; // 提示框一定要放在最后布局,先等文字高度确定之后看提示框的高度是否要调整
