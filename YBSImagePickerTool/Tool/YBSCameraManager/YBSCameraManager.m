...
#import "UIImage+YBSResize.h"

#define aYBSustingFocus @"aYBSustingFocus"
...
@interface YBSCameraManager () <AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
...
- (void)setFocusObserver:(BOOL)yes
{
    ...
    if (device && [device isFocusPointOfInterestSupported]) {
        if (yes) {
            [device addObserver:self forKeyPath:aYBSustingFocus options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:nil];
        }else{
            [device removeObserver:self forKeyPath:aYBSustingFocus context:nil];
        }
    }else{
        ShowAlert(@"你的设备没有照相机");
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:aYBSustingFocus]) {
        BOOL isAYBSustingFocus = [[change objectForKey:NSKeyValueChangeNewKey] boolValue];
        if (isAYBSustingFocus) {
